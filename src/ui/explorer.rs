...
pub fn unpin_start_menu() {
    debug!("Unpinning Start menu items...");
...
pub fn enable_powershell_execution() {
    debug!("Setting PowerShell execution policy to Unrestricted...");
    let _ = Command::new("powershell")
        .args([
            "-NoProfile",
            "-Command",
...
pub fn set_windows_terminal_default() {
    debug!("Setting Windows Terminal as default console...");
    let _ = Command::new("powershell")
...
pub fn remove_edge_shortcut() {
    debug!("Removing Edge desktop shortcut...");
