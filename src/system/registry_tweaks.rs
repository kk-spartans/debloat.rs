...
use windows::Win32::System::Registry::{
    HKEY, HKEY_CLASSES_ROOT, HKEY_CURRENT_USER, HKEY_LOCAL_MACHINE, KEY_SET_VALUE, REG_DWORD,
...
pub fn disable_wpbt() {
    unsafe {
        let key_path = w!(
            "SYSTEM\\CurrentControlSet\\Control\\DeviceGuard\\Scenarios\\WindowsPlatformBinaryTable"
        );
        let mut key = HKEY::default();
        if RegOpenKeyExW(
            HKEY_LOCAL_MACHINE,
