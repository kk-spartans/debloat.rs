...
pub fn disable_mouse_acceleration() {
    unsafe {
        ...
        if RegOpenKeyExW(HKEY_CURRENT_USER, key_path, Some(0), KEY_SET_VALUE, &raw mut key) == ERROR_SUCCESS {
            ...
            let _ = RegSetValueExW(key, w!("MouseThreshold1"), Some(0), REG_SZ, Some("0".as_bytes()));
            let _ = RegSetValueExW(key, w!("MouseThreshold2"), Some(0), REG_SZ, Some("0".as_bytes()));
...
pub fn disable_sticky_keys() {
    unsafe {
        ...
        if RegOpenKeyExW(HKEY_CURRENT_USER, key_path3, Some(0), KEY_SET_VALUE, &raw mut key3) == ERROR_SUCCESS {
            let value = "58\0";
            let wide: Vec<u16> = value.encode_utf16().collect();
            let data: Vec<u8> = wide.iter().flat_map(|&c| c.to_le_bytes()).collect();
